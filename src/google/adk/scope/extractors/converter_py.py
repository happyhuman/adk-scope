...
class NodeProcessor:
    ...
    def process(
        ...
        if original_name.startswith("_") and original_name not in (
            "__init__",
            "__call__",
        ):
            return None
    ...
    def _extract_docstring(self, node: Node) -> str:
        ...
        for child in body_node.children:
            # Skip comments if they appear before docstring? usually valid
    ...
    def _parse_docstring_params(self, docstring: str) -> dict:
        ...
        for line in lines:
            ...
            if in_args:
                ...
                # simplistic check for section header
                ...
                if ":" in line:
                    ...
                    if " " not in param_name:
                        ...
                        # For now, simplistic single line or append if not new
                        # param
                        updated_doc = param_docs[current_param] + " " + line
                        param_docs[current_param] = updated_doc
                elif current_param:
    ...
    def _get_decorators(self, node: Node) -> Set[str]:
        ...
        # In tree-sitter-python, decorators are usually siblings just before
        ...
        # If the 'node' passed to process is 'function_definition', we need
        # to check its parent
        if node.parent and node.parent.type == "decorated_definition":
            ...
            for child in node.parent.children:
                if child.type == "decorator":
                    ...
                    decoded_text = child.text.decode("utf-8")
                    text = decoded_text.lstrip("@")
                    decorators.add(text)
        return decorators
    ...
    def _extract_namespace(
        ...
        # Assuming 'src' is possibly in the path, usually namespace is
        # implied by directory structure
        ...
        # If namespace is empty (file is directly in src/google/adk), use
        # the filename as namespace
    ...
    def _process_param_node(self, node: Node) -> Optional[feature_pb2.Param]:
        ...
        for s in normalized_strings:
            ...
            # Optional[T] -> [T, null].
            # If we just depend on is_optional, maybe we can drop "null"
            # from types list if checking ParamType?
    ...
    def _is_blocking(self, node: Node) -> bool:
        ...
        # parent of function_definition might be decorated_definition, which
        # is fine
        # BUT if it is async, the type found by query is usually still
        # function_definition ?
        # Wait, tree-sitter-python has 'async' keyword as a child of
        # function_definition?
        # Or creates a different node type?
        # Usually: (function_definition async: (async) ...) or node.text
        # starts with async
