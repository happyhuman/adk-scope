...
def get_search_dir(input_path: Path, language: str) -> Path:
    ...
    logger.warning("Could not find standard source directories (%s) in %s. Scanning root.", subdirs, input_path)
    return input_path

def main():
    args = parse_args() # language is already normalized to "python" or "typescript"
    ...
    if args.input_file:
        ...
        for p in files:
            ...
            if features:
                 try:
                    ...
                    display_path = p.name
                 logger.info("File: %s - Found %d features", display_path, len(features))
        ...
        logger.info("Found %d %s files in %s.", len(files), args.language, search_dir)
    ...
    # Version extraction
    version = extractor_module.get_version(repo_root if repo_root else Path("."))
