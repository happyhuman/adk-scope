...
def generate_matrix_report_from_csvs(
    ...
    for csv_path in csv_paths:
        try:
            ...
            if len(namespaces) < 2:
                logging.warning(f"Skipping {csv_path}: Could not detect base/target languages.")
                continue
    ...
    try:
        ...
        logging.info(f"Successfully wrote matrix report from CSVs to {output_path}")
    except Exception as e:
...
def main():
    ...
    try:
        ...
        if is_matrix_input:
            ...
            if not all(p.endswith(".csv") for p in registry_paths):
                 logging.error("All inputs must be CSV files for CSV matrix report.")
                 sys.exit(1)
    ...
    generate_markdown_raw_reports(
        registries, output_path=output_path
    )
