...
class TestExtractMain(unittest.TestCase):
    def setUp(self):
        ...
        # Patches
        self.mock_args_patcher = patch('google.adk.scope.extractors.extract.parse_args')
        self.mock_args = self.mock_args_patcher.start()
        
        self.mock_extractors_patcher = patch.dict('google.adk.scope.extractors.extract.EXTRACTORS', clear=True)
    ...
    def configure_args(self, lang="python", input_file=None, input_dir=None, input_repo=None, output=None):
        mock_args = MagicMock()
    ...
    def test_input_repo_mode(self):
        ...
        self.mock_py_extractor.find_files.assert_called_with(r/"src", recursive=True)
        self.assertTrue(self.output_file.exists())
