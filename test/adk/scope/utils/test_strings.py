...
class TestStrings(unittest.TestCase):
    ...
    def test_normalize_type_complex(self):
        ...
        self.assertEqual(
            normalize_type_complex("list[int]"), ["LIST"]
        )  # lowercase

        # Generics - Union
        self.assertEqual(
            set(normalize_type_complex("Union[int, str]")), {"NUMBER", "STRING"}
        )

        # check explicit behavior for None/none in simple_normalize
        # _simple_normalize('none') -> 'null'
        self.assertEqual(
            set(normalize_type_complex("Union[int, None]")), {"NUMBER", "null"}
        )

        # Generics - Optional
        self.assertEqual(
            set(normalize_type_complex("Optional[int]")), {"NUMBER", "null"}
        )
        self.assertEqual(
            set(normalize_type_complex("Optional[List[str]]")), {"LIST", "null"}
        )

        # Generics - AsyncGenerator
        self.assertEqual(
            set(normalize_type_complex("AsyncGenerator[int, str]")),
            {"NUMBER", "STRING"},
        )

        # Generics - Tuple
        self.assertEqual(
            set(normalize_type_complex("tuple[int, str]")), {"NUMBER", "STRING"}
        )
        self.assertEqual(
            set(normalize_type_complex("Tuple[int, str]")), {"NUMBER", "STRING"}
        )
        ...
        # Nested generics
        self.assertEqual(
            set(normalize_type_complex("Union[List[int], Optional[str]]")),
            {"LIST", "STRING", "null"},
        )
    ...
    def test_split_generics(self):
        ...
        self.assertEqual(
            _split_generics("Union[A, B[C, D]], E"), ["Union[A, B[C, D]]", "E"]
        )
